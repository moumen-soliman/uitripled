...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@uitripled/ui/components/ui/tabs";
import { getComponentById } from "@/lib/components-registry";
import { categoryNames, uiLibraryLabels, type UILibrary } from "@uitripled/shared";
    ...
    if (component.category === "native") {
      const loadComponent = async () => {
        try {
          if (selectedLibrary === "baseui") {
            try {
              const baseuiModule = await import(
                `@uitripled/ui/components/native/base-ui/${component.id}-baseui`
              );
              ...
              if (baseuiModule[componentName]) {
                ...
                try {
                  const demoModule = await import(
                    `@uitripled/ui/components/native/base-ui/demo/${component.id}-demo`
                  );
            ...
            try {
              const carbonModule = await import(
                `@uitripled/ui/components/native/carbon/${component.id}-carbon`
              );
              ...
              if (carbonModule[componentName]) {
                try {
                  const demoModule = await import(
                    `@uitripled/ui/components/native/carbon/demo/${component.id}-demo`
                  );
                  const componentPrefix = component.id
                    .split("-")
                    .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
          ...
          const shadcnuiModule = await import(
            `@uitripled/ui/components/native/shadcn-ui/${component.id}-shadcnui`
          );
          ...
          if (shadcnuiModule[componentName]) {
            ...
            try {
              const demoModule = await import(
                `@uitripled/ui/components/native/shadcn-ui/demo/${component.id}-demo`
              );
      ...
      const loadSectionComponent = async () => {
        try {
          if (selectedLibrary === "baseui") {
            const baseuiModule = await import(
              `@uitripled/ui/components/sections/base-ui/${component.id}-baseui`
            );
            const exports = Object.keys(baseuiModule);
